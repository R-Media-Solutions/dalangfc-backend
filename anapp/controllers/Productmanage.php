...
class Productmanage extends Admin_Controller {
    ...
    function productlistsdata(){
        ...
        elseif( $column == 3 )  { $order_by .= 'price_member ' . $sort; }
        elseif( $column == 4 )  { $order_by .= 'price_customer ' . $sort; }
        elseif( $column == 5 )  { $order_by .= '%status% ' . $sort; }
        //elseif( $column == 8 )  { $order_by .= '%type% ' . $sort; }
        elseif( $column == 6 )  { $order_by .= '%dateupdated% ' . $sort; }
        
        //$data_list          = ( $is_admin ) ? $this->Model_Product->get_all_product($limit, $offset, $condition, $order_by, $params) : array();
        $data_list          = $this->Model_Product->get_all_product($limit, $offset, $condition, $order_by, $params);
        $records            = array();
    ...
    function categorylistsdata(){
        ...
        $data_list          = $this->Model_Product->get_all_category($limit, $offset, $condition, $order_by);
        $records            = array();
        $records["aaData"]  = array();
        ...
        if( !empty($data_list) ){
            ...
            foreach($data_list as $row){
                ...
                $btn_edit       = '<a href="'.base_url('productmanage/savecategory/'.$id).'" class="btn btn-sm btn-default btn-tooltip btn-edit-category" title="Edit Kategori" data-category="'.$row->name.'"><i class="fa fa-edit"></i></a>';
                $btn_delete     = '<a href="javascript:;" 
