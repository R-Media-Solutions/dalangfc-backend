<?php 
    $data_services   = isset($data_services) ? $data_services : false; 
    $discount_type  = config_item('discount_type');
?>

<div class="header bg-secondary pb-6">
    <div class="container-fluid">
        <div class="header-body">
            <div class="row align-items-center py-4">
                <div class="col-lg-6 col-7">
                    <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
                        <ol class="breadcrumb breadcrumb-links breadcrumb-light">
                            <li class="breadcrumb-item"><a href="<?php echo base_url('dashboard') ?>"><i class="fas fa-home"></i></a></li>
                            <li class="breadcrumb-item"><a href="#"><?php echo lang('menu_services') ?></a></li>
                            <li class="breadcrumb-item active" aria-current="page">Form</li>
                        </ol>
                    </nav>
                </div>
            </div>
        </div>
    </div>
</div>

<div class="container-fluid mt--6">
    <div class="row">
        <div class="col-xl-12">
            <div class="row justify-content-center">
                <div class="col-lg-12 card-wrapper">
                    <div class="card">
                        <div class="card-header">
                            <div class="row align-items-center">
                                <div class="col-8">
                                    <h3 class="mb-1"><?php echo $form_title; ?> </h3>
                                    <?php if ( $form_page == 'edit' ) { ?>
                                        <h5 class="text-muted mb-0"><?php echo $data_services->name; ?> </h5>
                                    <?php } ?>
                                </div>
                                <div class="col-4 text-right">
                                    <?php if ( $form_page == 'edit' ) { ?>
                                        <a href="<?php echo base_url('services/serviceslists') ?>" class="btn btn-sm btn-danger"><span class="fa fa-history"></span> Kembali</a>
                                    <?php } else { ?>
                                        <a href="<?php echo base_url('services/serviceslists') ?>" class="btn btn-sm btn-outline-default"><span class="fa fa-list"></span> List Data Layanan</a>
                                    <?php } ?>
                                </div>
                            </div>
                        </div>
                        <div class="card-body wrapper-form-services pt-0">
                            <?php 
                                $form_action = base_url('services/saveservices');
                                if ( $form_page == 'edit' ) { 
                                    $form_action .= '/'. an_encrypt($data_services->id);
                                }
                            ?>
                            <form role="form" method="post" action="<?php echo $form_action; ?>" id="form-services" class="form-horizontal">
                                <div class="pl-lg-4">
                                    <div class="row">
                                        <div class="col-xl-8 order-xl-2 pt-3">
                                            <div class="form-group">
                                                <label class="form-control-label" for="services_name"><?php echo lang('name'); ?> <span class="required">*</span></label>
                                                <div class="input-group input-group-merge input-group-alternative">
                                                    <div class="input-group-prepend">
                                                        <span class="input-group-text"><i class="ni ni-box-2"></i></span>
                                                    </div>
                                                    <input type="text" name="services_name" id="services_name" class="form-control" placeholder="<?php echo lang('services_name'); ?>" value="<?php echo( $data_services ? $data_services->name : ''); ?>">
                                                </div>
                                            </div>
                                            <!--
                                            <div class="form-group">
                                                <label class="form-control-label" for="services_category"><?php echo lang('category'); ?> <span class="required">*</span></label>
                                                <div class="input-group">
                                                    <select class="form-control" name="services_category" id="services_category">
                                                        <option value="" disabled="" selected="">-- <?php echo lang('select').' '.lang('category'); ?> --</option>
                                                        <?php
                                                            if ( $product_category = an_product_category(0, true) ) {
                                                                foreach ($product_category as $key => $row) {
                                                                    $selected = '';
                                                                    if ( $data_services ) {
                                                                        if ( $data_services->id_category == $row->id ) {
                                                                            $selected = 'selected=""';
                                                                        } else {
                                                                            $selected = '';
                                                                        }
                                                                    }
                                                                    echo '<option value="'. $row->id .'" '.$selected.'>'. ucwords($row->name) .'</option>';
                                                                }   
                                                            }
                                                        ?>
                                                    </select>
                                                    <div class="input-group-append">
                                                        <button class="btn btn-outline-default" type="button" id="btn-modal-category">
                                                            <i class="fa fa-plus"></i> <?php echo lang('category'); ?>
                                                        </button>
                                                    </div>
                                                </div>
                                            </div>
                                            -->
                                            <!--
                                            <div class="form-group">
                                                <label class="form-control-label" for="short_description"><?php echo lang('short_description'); ?> </label>
                                                <div class="input-group input-group-merge input-group-alternative">
                                                    <div class="input-group-prepend">
                                                        <span class="input-group-text"><i class="ni ni-file"></i></span>
                                                    </div>
                                                    <input type="text" name="short_description" id="short_description" class="form-control" placeholder="<?php echo lang('short_description'); ?>" value="<?php echo( $data_services ? $data_services->short_description : ''); ?>">
                                                </div>
                                            </div>
                                            <div class="form-group">
                                                <label class="form-control-label" for="editor">Deskripsi</label>
                                                <div id="editor-services" data-quill-placeholder="Deskripsi">
                                                    <?php echo ( $data_services ? $data_services->description : ''); ?>
                                                </div>
                                            </div>
                                            -->
                                        </div>
                                        <div class="col-xl-4 order-xl-1 bg-secondary py-2">
                                            <div class="form-group">
                                                <?php 
                                                    $file_src   = ASSET_PATH . 'backend/img/no_image.jpg'; 
                                                    if ( $data_services ) {
                                                        if ( $data_services->image ) {
                                                            $file_path = SERVICES_IMG_PATH . $data_services->image;
                                                            if ( file_exists($file_path) ) {
                                                                $file_src = SERVICES_IMG . $data_services->image;
                                                            }
                                                        }
                                                    }

                                                ?>
                                                <div class="thumbnail mb-1">
                                                    <img class="img-thumbnail" id="services_img_thumbnail" width="100%" src="<?php echo $file_src; ?>" style="cursor: pointer;">
                                                    <div class="caption">
                                                        <p class="text-muted mb-0" style="font-size: 14px">Image ( jpg, jpeg, png ) and Max 2 MB</p>
                                                        <div class="img-information" style="display: none;">
                                                            <i class="ni ni-album-2 mr-1" id="type_img_thumbnail"></i> 
                                                            <span id="size_img_thumbnail"></span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <input type="file" name="services_file" id="services_file" class="form-control file-image" accept="image/x-png,image/jpeg">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <hr class="my-4" />
                                <div class="text-center">
                                    <button type="submit" class="btn btn-primary bg-gradient-default my-2"><?php echo lang('services_save'); ?></button>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>

<!-- Modal Add Category -->
<div class="modal fade" id="modal-add-category" tabindex="-1" role="dialog" aria-labelledby="modal-add-category" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="fa fa-plus"></i> <?php echo lang('category'); ?></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form role="form" method="post" action="<?php echo base_url('productmanage/savecategory'); ?>" id="form-category" class="form-horizontal">
                <input type="hidden" name="form" class="d-none" value="product" />
                <div class="modal-body wrapper-form-category">
                    <div class="form-group">
                        <label class="form-control-label" for="category"><?php echo lang('category'); ?> <span class="required">*</span></label>
                        <input type="text" id="category" name="category" class="form-control" placeholder="<?php echo lang('category'); ?>" />
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal"><?php echo lang('back'); ?></button>
                    <button type="submit" class="btn btn-default"><?php echo lang('save'); ?></button>
                </div>
            </form>
        </div>
    </div>
</div>
