...
if (!function_exists('an_client')) {
     ...
     * @return  Data
     */
    function an_client($id = '', $is_active = false)
    {
        $CI = &get_instance();
        $client = $CI->Model_Home->get_client($id, $is_active);
        return $client;
    }
}
...
if (!function_exists('an_client_by')) {
     ...
     * @return  Data
     */
    function an_client_by($field = '', $value = '', $conditions = '', $limit = 0)
    {
        $CI = &get_instance();
        $client = $CI->Model_Home->get_client_by($field, $value, $conditions, $limit);
        return $client;
    }
}
...
|--------------------------------------------------------------------------
*/
if (!function_exists('an_idcard_image')) {
...
if (!function_exists('an_services_image')) {
    function an_services_image($image = '')
    {
        $CI = &get_instance();
        $no_image = ASSET_PATH . 'backend/img/no_image.jpg';
        if ($image) {
            $img_src    = SERVICES_IMG_PATH . $image;
            if (file_exists($img_src)) {
                $img_src = SERVICES_IMG . $image;
            } else {
                $img_src = $no_image;
            }
            return $img_src;
...
if (!function_exists('an_client_image')) {
    function an_client_image($image = '')
    {
        $CI = &get_instance();
        $no_image = ASSET_PATH . 'backend/img/no_image.jpg';
        if ($image) {
            $img_src    = CLIENT_IMG_PATH . $image;
            if (file_exists($img_src)) {
...
if (!function_exists('an_aboutus_history')) {
    /**
     * Get About Us History data
     * @author  Rifal
     * @param   Int     $id     (Optional)  history id
     * @return  Data
     */
    function an_aboutus_history($id = '', $is_active = false)
    {
        $CI         = &get_instance();
        $historys   = $CI->Model_Aboutus->get_aboutus_history($id, $is_active);
        return $historys;
    }
}

if (!function_exists('an_aboutus_history_image')) {
    function an_aboutus_history_image($image = '', $thumbnail = true)
    {
        $CI = &get_instance();
        ...
        if ($image) {
            $thumb_path = $thumbnail ? 'thumbnail/' : '';
            $img_src    = ABOUTHISTORY_IMG_PATH . $thumb_path . $image;
            if (file_exists($img_src)) {
                $img_src = ABOUTHISTORY_IMG . $thumb_path . $image;
            } else {
                $img_src = $no_image;
            }
            return $img_src;
...
if (!function_exists('an_services')) {
    /**
     * Get services data
     * @author  Rifal
     * @param   Int     $id     (Optional)  services id
     * @return  Data
     */
    function an_services($id = '', $is_active = false)
    {
        $CI = &get_instance();
        $services = $CI->Model_Services->get_services($id, $is_active);
        return $services;
    }
}

if (!function_exists('an_services_by')) {
    /**
     * Get services by field data
     * @author  Rifal
     * @param   String     $field     (Required)  field of data
     * @param   String     $value     (Required)  value of field
     * @return  Data
     */
    function an_services_by($field = '', $value = '', $conditions = '', $limit = 0)
    {
        $CI = &get_instance();
        $services = $CI->Model_Services->get_services_by($field, $value, $conditions, $limit);
        return $services;
    }
}
...
if (!function_exists('an_year')) {
    ...
    {
        $CI     = &get_instance();
        $years  = $CI->Model_Option->get_year($id);
        return $years;
    }
}
...
-----------------------------------------------
1.0.0   2016/06/01  Rifal           - Create this changelog.
*/
