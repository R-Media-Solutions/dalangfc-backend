...
// ===========================================================
// ABoutUs Setting Function
// ===========================================================
var AbouUsManage = function() {
    var history_img;
    ...
    // Quill Editor Load
    // ---------------------------------
    var text_editor         = $('#editor-aboutuhistory');
    var placeholder_editor  = $('#editor-aboutuhistory').data("quill-placeholder");
    if ( text_editor.length ) {
        var quill_editor    = new Quill('#editor-aboutuhistory', {
            modules: {
                toolbar: [
                    ["bold", "italic"],
                    ["link", "blockquote", "code"],
    ...
    // ABoutUs Setting Update
    var handleSaveAboutUs = function(url, value){
        if ( ! url || url == undefined || value == undefined ) { 
            return; 
        }
        $.ajax({
    ...
    // ABout Us Setting Function
    var handleAboutusSetting = function() {
        // Update ABoutUs
        // -----------------------------------------------
        $('button.general-setting').click(function(e){
            e.preventDefault();
        ...
        $('button.general-setting-each').click(function(e){
            ...
            bootbox.confirm("Apakah anda yakin akan simpan data ini ?", function(result) {
                if( result == true ){
                    $.ajax({
                        type:   "POST",
                        url:    url,
        ...
        // Button Action Get Data ABoutUs Detail 
        // -----------------------------------------------
        $("body").delegate( "a.detaildata", "click", function( e ) {
            e.preventDefault();
            var url     = $(this).attr('href');
            var wrapper = $('#aboutus_detail_list').parents('.dataTables_wrapper');
            var m_edit  = $('#modal-form-abouusdetail');
            var m_view  = $('#abouusdetail_view_modal');
            var _form   = $('#form_aboutusdetail_edit');
            ...
            $.ajax({
                ...
                success: function( response ){
                    ...
                    if( response.status == 'login' ){
                        ...
                        if( response.status == 'success'){
                            if ( response.process == 'edit' ) {
                                $('#aboutusdetail_edit_title').text(response.notification.name);
                                $('#aboutusdetail_id', _form).val(response.notification.id);
                                $('#aboutusdetail_type', _form).val(response.notification.type);
                                $('#aboutusdetail_title', _form).val(response.notification.title);
                                $('#aboutusdetail_status', _form).val(response.notification.status);
                                $('#aboutusdetail_content_plain', _form).val(response.notification.content);
                                CKEDITOR.instances['aboutusdetail_content_email'].setData( response.notification.content );
                                CKEDITOR.instances['aboutusdetail_content_email'].resize(CKEDITOR.instances['aboutusdetail_content_email'].width, 300);
                                if ( response.notification.type == 'email' ) {
                                    $('#content_email', _form).show();
                                    $('#content_plain', _form).hide();
                                    $('#aboutusdetail_edit_type').text('Email');
                                    $('#aboutusdetail_edit_color').removeClass('label-success').addClass('label-primary');
                                    $('#aboutusdetail_edit_icon').removeClass('fa-whatsapp').addClass('fa-envelope');
                                } else {
                                    $('#content_email', _form).hide();
                                    $('#content_plain', _form).show();
                                    $('#aboutusdetail_edit_type').text('WhatsApp');
                                    $('#aboutusdetail_edit_color').removeClass('label-primary').addClass('label-success');
                                    $('#aboutusdetail_edit_icon').removeClass('fa-envelope').addClass('fa-whatsapp');
                                }
                                m_edit.modal('show');
                            } else {
                                $('#aboutusdetail_view_title').text(response.notification.title);
                                $('#aboutusdetail_view_content').html(response.notification.content);
                                if ( response.notification.type == 'email' ) {
                                    $('#aboutusdetail_view_type').text('Email');
                                    $('#aboutusdetail_view_color').removeClass('label-success').addClass('label-primary');
                                    $('#aboutusdetail_view_icon').removeClass('fa-whatsapp').addClass('fa-envelope');
                                }else{
                                    $('#aboutusdetail_view_type').text('WhatsApp');
                                    $('#aboutusdetail_view_color').removeClass('label-primary').addClass('label-success');
                                    $('#aboutusdetail_view_icon').removeClass('fa-envelope').addClass('fa-whatsapp');
                                }
                                m_view.modal('show');
                            }
                        }else{
        ...
        // -----------------------------------------------
        $("body").delegate( "a.btn-status-detailaboutus", "click", function( e ) {
            e.preventDefault();
            var url         = $(this).attr('href');
            var detail      = $(this).data('detail');
            var status      = $(this).data('status');
                        ...
                        success: function( response ){
                            ...
                            if( response.status == 'access_denied' ){
                                ...
                                if( response.status == 'success'){
                                    ...
                                    $('#btn_aboutdetail_list').trigger('click');
                                }else{
                                    App.notify({
                                        icon: 'fa fa-exclamation-triangle', 
                                        title: 'Failed', 
    ...
    // ---------------------------------
    // Handle General About Us History Manage
    // ---------------------------------
    var handleGeneralAboutusHistory = function() {
        $('#aboutushistory_img_thumbnail').on('click', function(e) {
            $('.file-image').trigger('click');
        });

        $('#aboutushistory_file, .file-image').on('change', function(e) {
            readURL( $(this), $('#aboutushistory_img_thumbnail') );
            history_img = e.target.files;
        });
        ...
        // Button Edit Status History Data
        // -----------------------------------------------
        $("body").delegate( "a.btn-status-history", "click", function( e ) {
            e.preventDefault();
            var url         = $(this).attr('href');
            ...
            var msg_body    = `
                <div class="row pt-5 align-items-center">
                    ...
                    <div class="col-sm-12 text-center">
                        <small class="text-uppercase text-muted font-weight-bold">Data : </small>
                        <h2 class="heading-title text-primary mb-0">`+ history +`</h2>
                    </div>
                </div>`;
            bootbox.confirm(msg_body, function(result) {
                if( result == true ){
                        ...
                        success: function( response ){
                            ...
                            if( response.status == 'access_denied' ){
                                ...
                                if( response.status == 'success'){
                                    ...
                                    $('#btn_list_table_abouthistory').trigger('click');
                                }else{
                                    App.notify({
                                        icon: 'fa fa-exclamation-triangle', 
                                        title: 'Failed', 
        ...
        // Button Delete History Data
        // -----------------------------------------------
        $("body").delegate( "a.btn-delete-history", "click", function( e ) {
            e.preventDefault();
            var url         = $(this).data('url');
            var history     = $(this).data('history');
            ...
            var msg_body    = `
                <div class="row pt-5 align-items-center">
                    <div class="col-sm-12">
                        <h3 class="heading mb-3 text-center">Apakah anda yakin akan Meng-Hapus Data Ini ?</h3>
                    </div>
                    <div class="col-sm-12 text-center">
                        <small class="text-uppercase text-muted font-weight-bold">Data : </small>
                        <h2 class="heading-title text-primary mb-0">`+ history +`</h2>
    ...
    // ---------------------------------
    // Handle Validation About Us History Manage
    // ---------------------------------
    var handleValidationAboutusHistoryManage = function() {
        var form            = $('#form-aboutushistory');
        var wrapper         = $('.wrapper-form-aboutushistory');
        ...
        form.validate({
            ...
            messages: {
                aboutushistory_name: {
                    required: "Nama Sejarah harus di isi !",
                    minlength: "Minimal 3 karakter"
                },
            },
            errorPlacement: function (error, element) { // render error placement for each input type
            ...
            submitHandler: function (form) {
                ...
                if (history_img) {
                    $.each(history_img, function(key, value){
                        data.append('history_img', value);
                    });
                }
    ...
    return {
        init: function() {
            handleSaveAboutUs();
            handleAboutusSetting();
            handleValidationAboutusHistoryManage();
            handleGeneralAboutusHistory();
        },
    };
}();
...
// ===========================================================
var ServicesManage = function() {
    var services_img;

    var total_qty       = 0;
    var total_price     = 0;

    var _trEmpty        = `<tr class="data-empty"><td colspan="5" class="text-center">Data layanan belum ada yang di pilih.</td></tr>`;

    // ---------------------------------
    // Quill Editor Load
    // ---------------------------------
    var text_editor         = $('#editor-services');
    var placeholder_editor  = $('#editor-services').data("quill-placeholder");
    if ( text_editor.length ) {
        var quill_editor    = new Quill('#editor-services', {
            modules: {
                toolbar: [
                    ["bold", "italic"],
                    ["link", "blockquote", "code"],
    ...
    // ---------------------------------
    // Handle General Services Manage
    // ---------------------------------
    var handleGeneralServicesManage = function() {
        $('#services_img_thumbnail').on('click', function(e) {
            $('.file-image').trigger('click');
        });

        $('#services_file, .file-image').on('change', function(e) {
            readURL( $(this), $('#services_img_thumbnail') );
            services_img = e.target.files;
        });
        ...
        // Button Edit Status Services Data
        // -----------------------------------------------
        $("body").delegate( "a.btn-status-services", "click", function( e ) {
            e.preventDefault();
            var url         = $(this).attr('href');
            ...
            var msg_body    = `
                <div class="row pt-5 align-items-center">
                    ...
                    <div class="col-sm-12 text-center">
                        <small class="text-uppercase text-muted font-weight-bold">Data : </small>
                        <h2 class="heading-title text-primary mb-0">`+ services +`</h2>
                    </div>
                </div>`;
            bootbox.confirm(msg_body, function(result) {
                if( result == true ){
                        ...
                        success: function( response ){
                            ...
                            if( response.status == 'access_denied' ){
                                ...
                                if( response.status == 'success'){
                                    ...
                                    $('#btn_list_table_services').trigger('click');
                                }else{
                                    App.notify({
                                        icon: 'fa fa-exclamation-triangle', 
                                        title: 'Failed', 
        ...
        // Button Delete Services Data
        // -----------------------------------------------
        $("body").delegate( "a.btn-delete-services", "click", function( e ) {
            e.preventDefault();
            var url         = $(this).data('url');
            var services    = $(this).data('services');
    ...
    // ---------------------------------
    // Handle Validation Services Manage
    // ---------------------------------
    var handleValidationServicesManage = function() {
        var form            = $('#form-services');
        var wrapper         = $('.wrapper-form-services');
                ...
                */
                ...
                if (services_img) {
                    $.each(services_img, function(key, value){
                        data.append('services_img', value);
                    });
                }
    ...
    return {
        init: function() {
            handleGeneralServicesManage();
            handleValidationServicesManage();
        },
    };
}();

// ===========================================================
// Home manage Function
// ===========================================================
var HomeManage = function() {
    ...
    // Home Manage Update
    var handleSaveHome = function(url, value){
        if ( ! url || url == undefined || value == undefined ) { 
            return; 
        }
        $.ajax({
    ...
    var handleHomeSetting = function() {
        ...
        $("body").delegate( "a.detaildata", "click", function( e ) {
            e.preventDefault();
            var url     = $(this).attr('href');
            var wrapper = $('#homedetail_detail_list').parents('.dataTables_wrapper');
            var m_edit  = $('#modal-form-homedetail');
            var m_view  = $('#homedetail_view_modal');
            var _form   = $('#form_homedetail_edit');
            ...
            $.ajax({
                ...
                success: function( response ){
                    ...
                    if( response.status == 'login' ){
                        ...
                        if( response.status == 'success'){
                            if ( response.process == 'edit' ) {
                                $('#homedetail_edit_title').text(response.notification.name);
                                ...
                                CKEDITOR.instances['homedetail_content_email'].resize(CKEDITOR.instances['homedetail_content_email'].width, 300);
                                if ( response.notification.type == 'email' ) {
                                    $('#content_email', _form).show();
                                    $('#content_plain', _form).hide();
                                    $('#homedetail_edit_type').text('Email');
                                    $('#homedetail_edit_color').removeClass('label-success').addClass('label-primary');
                                    $('#homedetail_edit_icon').removeClass('fa-whatsapp').addClass('fa-envelope');
                                } else {
                                    $('#content_email', _form).hide();
                                    $('#content_plain', _form).show();
                                    $('#homedetail_edit_type').text('WhatsApp');
                                    $('#homedetail_edit_color').removeClass('label-primary').addClass('label-success');
                                    $('#homedetail_edit_icon').removeClass('fa-envelope').addClass('fa-whatsapp');
                                }
                                m_edit.modal('show');
                            } else {
                                $('#homedetail_view_title').text(response.notification.title);
                                $('#homedetail_view_content').html(response.notification.content);
                                if ( response.notification.type == 'email' ) {
                                    $('#homedetail_view_type').text('Email');
                                    $('#homedetail_view_color').removeClass('label-success').addClass('label-primary');
                                    $('#homedetail_view_icon').removeClass('fa-whatsapp').addClass('fa-envelope');
                                }else{
                                    $('#homedetail_view_type').text('WhatsApp');
                                    $('#homedetail_view_color').removeClass('label-primary').addClass('label-success');
                                    $('#homedetail_view_icon').removeClass('fa-envelope').addClass('fa-whatsapp');
                                }
                                m_view.modal('show');
                            }
                        }else{
    ...
    // ---------------------------------
    var handleGeneralClientManage = function() {
        $('#client_img_thumbnail').on('click', function(e) {
            $('.file-image').trigger('click');
        });

        $('#client_file, .file-image').on('change', function(e) {
            readURL( $(this), $('#client_img_thumbnail') );
            client_img = e.target.files;
        });

        // Button Edit Status Client Data
        // -----------------------------------------------
        $("body").delegate( "a.btn-status-client", "click", function( e ) {
            e.preventDefault();
            var url         = $(this).attr('href');
            var client      = $(this).data('client');
            var status      = $(this).data('status');
            var msg_title   = (status == '1') ? 'Apakah anda yakin akan Meng-Nonaktifkan Data Ini ?' : 'Apakah anda yakin akan Meng-Aktifkan Produk Ini ?';
            ...
            var msg_body    = `
                <div class="row pt-5 align-items-center">
                    ...
                    <div class="col-sm-12 text-center">
                        <small class="text-uppercase text-muted font-weight-bold">Data : </small>
                        <h2 class="heading-title text-primary mb-0">`+ client +`</h2>
                    </div>
                </div>`;
            bootbox.confirm(msg_body, function(result) {
                if( result == true ){
                        ...
                        success: function( response ){
                            ...
                            if( response.status == 'access_denied' ){
                                ...
                                if( response.status == 'success'){
                                    ...
                                    $('#btn_list_table_client').trigger('click');
                                }else{
                                    App.notify({
                                        icon: 'fa fa-exclamation-triangle', 
                                        title: 'Failed', 
        ...
        // Button Delete Client Data
        // -----------------------------------------------
        $("body").delegate( "a.btn-delete-client", "click", function( e ) {
            e.preventDefault();
            var url         = $(this).data('url');
            var client      = $(this).data('client');
    ...
    // ---------------------------------
    // Handle Validation Client Manage
    // ---------------------------------
    var handleValidationClientManage = function() {
        var form            = $('#form-client');
        var wrapper         = $('.wrapper-form-client');
        ...
        form.validate({
            ...
            submitHandler: function (form) {
                ...
                bootbox.confirm("Apakah anda yakin akan simpan data pelanggan ini ?", function(result) {
                    if( result == true ){
                        $.ajax({
                            type:   "POST",
                            url:    url,
